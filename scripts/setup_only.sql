...
-- Lab warehouse
CREATE WAREHOUSE IF NOT EXISTS DQ_LAB_WH
    WAREHOUSE_SIZE = 'X-SMALL'
    AUTO_SUSPEND = 60
    AUTO_RESUME = TRUE
    INITIALLY_SUSPENDED = FALSE
    COMMENT = 'Dedicated warehouse for DQ Monitoring HOL';

-- Roles
CREATE ROLE IF NOT EXISTS CORP_DQ_ADMIN;
GRANT USAGE ON DATABASE CORP_DWH TO ROLE CORP_DQ_ADMIN;
GRANT CREATE SCHEMA ON DATABASE CORP_DWH TO ROLE CORP_DQ_ADMIN;
...
GRANT USAGE ON WAREHOUSE DQ_LAB_WH TO ROLE CORP_DQ_ADMIN;
...
GRANT USAGE ON WAREHOUSE DQ_LAB_WH TO ROLE CORP_DQ_STEWARD;
